...
namespace ConsoleTextInterface.Storage.Converters.Hotel
{
    public class HotelConverter : IConverter<HotelModel>
    {
        ...
        public HotelModel Deserialize(string value)
        {
            ...
            return new HotelModel() { Id = values[0].Trim(), Name = values[1].Trim(), City = values[2].Trim(), Capacity = int.Parse(values[3].Trim()), Price = double.Parse(values[4].Trim()) };
        }

        public string Serialize(HotelModel value)
        {
            return $"{value.Id}, {value.Name}, {value.City}, {value.Capacity}, {value.Price}";
        }
    }
